...
test('a host key that does not match the pinned fingerprint is refused', async () => {
  ...
  assert.match(result.message, /does not match the configured fingerprint/);
  assert.match(result.message, /connection was refused/);
  await adapter.dispose();
  await stop();
  void server;
});
...
test('without a pinned fingerprint the connection is refused, not silently trusted', async () => {
  ...
  assert.equal(result.ok, false);
  assert.match(result.message, /No SSH host key fingerprint is configured/);
