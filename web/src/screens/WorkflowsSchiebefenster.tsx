...
interface Props {
  canManage: boolean;
  onSchliessen(): void;
  /** Einen Workflow aufschlagen - `'new'` legt einen neuen an. */
  onOeffnen(jobId: string | 'new'): void;
}
 ...
 */
export function WorkflowsSchiebefenster({ canManage, onSchliessen, onOeffnen }: Props) {
              ...
              {gezeigt.map((job) => {
                ...
                return (
                  <tr
                    ...
                    onDoubleClick={() => onOeffnen(job.id)}
                  >
                    <td>
